...
class PKService {
  ...
  ValueNotifier<bool> isPrimaryHost = ValueNotifier(false);
  ...
  Future<ZIMCallInvitationSentResult> sendPKBattlesStartRequest(
    ...
    final requestData = <String, dynamic>{
      ...
      'avatar': ZEGOSDKManager().currentUser?.avatarUrlNotifier.value
    };
  ...
  void startSendSEI() {
    seiTimer = Timer.periodic(const Duration(milliseconds: 2500), (timer) {
      ...
      final seiData = <String, dynamic>{
        ...
        'sender_id': currentUser!=null ? currentUser.userID : '',
        'mic': currentUser!=null ? currentUser.isMicOnNotifier.value : false,
        'cam': currentUser!=null ? currentUser.isCamerOnNotifier.value: false
      };
  ...
  void onReceiveZegoUserRequest(IncomingUserRequestReceivedEvent event) {
    ...
    switch (type) {
      case PKProtocolType.startPK:
        ...
        invitation.inviterAvatar = invitationMap['avatar'];
        invitation.inviterID = event.info.inviter;
        currentZegoUserRequest = invitation;
        incomingPKRequestStreamCtrl
            .add(IncomingPKRequestEvent(invitation,requestID: event.requestID));
        break;
  ...
  void _onRoomAttributeUpdate(ZIMRoomAttributesUpdateInfo updateInfo) {
    print("pk_user_id${updateInfo.roomAttributes['pk_user_id']}");
    print("pk_seq${updateInfo.roomAttributes['pk_seq']}");
    print("pk_user_name${updateInfo.roomAttributes['pk_user_name']}");
  ...
  void onReceiveAudioFirstFrame(ZegoRecvAudioFirstFrameEvent event) {
    if (event.streamID.endsWith('_mix')) {
      print("streamidAudioframe${event.streamID}");
      print("streamidAudioframe${event.streamID}");
      print("streamidAudioframe${event.streamID}");
      print("hostNoti${ZegoLiveStreamingManager.instance.hostNoti.value!.userName}");
      print("currentuserrole${ZegoLiveStreamingManager.instance.currentUserRoleNoti.value.name}");
  ...
  void onReceiveVideoFirstFrame(ZegoRecvVideoFirstFrameEvent event) {
    if (event.streamID.endsWith('_mix')) {
      print("streamidvideoframe${event.streamID}");
      print("streamidvideoframe${event.streamID}");
      print("streamidvideoframe${event.streamID}");
      print("hostNoti${ZegoLiveStreamingManager.instance.hostNoti.value!.userName}");
      print("currentuserrole${ZegoLiveStreamingManager.instance.currentUserRoleNoti.value.name}");
